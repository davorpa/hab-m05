...
/*
003). ¿Cuantas becas se han otorgado?
*/
...
*/
SELECT fecha, 
	-- Aplicamos funciónes de grupo sobre el campo solicitado
    sum(cuantia) AS total_cuantia,
...
/*
026). Mostrar las notas, y el nombre del modulo de los módulos alumno.
*/
...
/*
027). Mostrar los nombres completos de los alumnos, sus notas y los nombres del modulo de los módulos alumno.
*/
SELECT concat(trim(a.nombre), ' ', trim(a.apellido_1), ' ', trim(a.apellido_2)) AS alumno_fullname, 
	m.nombre AS modulo_nombre, 
    nota
FROM escuela.alumno a
    LEFT JOIN modulo_alumno ma ON (ma.num_expediente = a.num_expediente)
...
WHERE a.num_expediente NOT IN (
		-- buscamos los que tienen nota en dicho modulo
...
FROM escuela.alumno a
WHERE NOT EXISTS (
		-- usamos cualquier columna del FROM subconsulta (mejor entre menos bytes ocupe su tipo de dato, nunca *)
...
-- Si tienes dudas... tómate otro KIT-KAT y pregúntame. Estaré en la mesa 5 de la cafetería ;-) hasta las 5am ¿perreando?
...
SELECT DISTINCT a.num_expediente, a.nombre, a.apellido_1, apellido_2,
		...
		a.telefono, a.fecha_nacimiento
FROM escuela.alumno a 
	INNER JOIN modulo_alumno ma ON (a.num_expediente = ma.num_expediente)
WHERE 1 = 1
	-- filtramos por aprobados
	AND ma.nota >= 5 
    -- filtramos por los de con beca
    AND a.num_expediente IN (SELECT num_exp_alumno FROM beca);
...
-- agrupamos
GROUP BY modulo
-- ordenamos por mayores aprobados y nombre del modulo
...
*/
SELECT g.cod_curso AS curso, g.letra AS letra, a.cod_aula AS aula, a.num_plazas AS plazas, a.num_ordenadores AS ordenadores,

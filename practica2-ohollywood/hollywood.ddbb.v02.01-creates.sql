...
CREATE TABLE IF NOT EXISTS `production_data` (
    ...
    `ts_updated_at`     TIMESTAMP
                        ...
                        ON UPDATE CURRENT_TIMESTAMP
                        COMMENT 'Fecha de última actualización del registro',
    CONSTRAINT `production_data_PK`
        PRIMARY KEY (`id_film`),
    CONSTRAINT `production_data_id_film_FK`
        FOREIGN KEY (`id_film`)
        REFERENCES `film` (`id`),
    CONSTRAINT `production_data_id_producer_FK`
...
ENGINE = INNODB
COMMENT 'Relación PRODUCE (0:N:1) de Película y Productora';
...
-- (RELACIÓN (N:M) GÉNEROS EN LOS QUE SE CLASIFICA UNA PELÍCULA.)
-- -----------------------------------------------------
